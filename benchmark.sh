...
temp_plot() {
  ...
  echo "set title \"Algoritmo RoundRobin N=$N Q=$Q I=100 [Intel Celeron 2597U]\n$2 vs $3\"" >> tmp.plt
  ...
  then
    ...
    echo "plot '_rb.dat' u $4:$5 w p t 'Ruby',\\" >> tmp.plt 
    echo "'_py.dat' u $4:$5 w p t 'Python',\\" >> tmp.plt
    echo "'_ja.dat' u $4:$5 w p t 'Java',\\" >> tmp.plt
    echo "'_cr.dat' u $4:$5 w p t 'Crystal'" >> tmp.plt
  fi
  gnuplot -c tmp.plt
}
...
for h in $(seq 5 5 10) # Procesos
do
  N=$h
  for i in 5 10 # Quantum
  do
    Q=$i
